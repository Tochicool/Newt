...
class Body:
    ...
    def constraintCollide(self, constraint, preserveImpulse):
        ...
        if length < target:
            if not length == 0:
                factor = 0.5*((length-target)/length*2)
            else:
                factor = 0

            if constraint.point1.anchored:
                factor *= 1.4
            if constraint.point2.anchored:
                factor *= 1.4
            if not self.anchored:
                self.p -= d*factor

            constraint.point1.correct(d*factor)
            constraint.point2.correct(d*factor)

            damping = (self.e +(constraint.point1.e+constraint.point2.e)*0.5)*0.5

            if preserveImpulse and length != 0:
                v1 = self.p - self.pp
                v2 = (constraint.point1.p+constraint.point2.p)/2 - (constraint.point1.pp+constraint.point2.pp)/2
                ...
                self.pp = self.p - v1
                constraint.point1.pp = constraint.point1.p - (v2/2)
                constraint.point2.pp = constraint.point1.p - (v2/2)
...
class rigidBody:
    edges = []
    centreOfMass = pygame.math.Vector2(0,0)
    
    def __init__(self, mass, edges):
        self.edges = edges
        self.mass = mass
        for edge in edges:
            self.centreOfMass += edge
        self.centreOfMass /= len(edges)
        
    def draw(self):
        fill = (165, 165, 165)
        pygame.draw.polygon(screen.surface, fill, self.edges)
...
def __main__():
    ...
    thing = rigidBody(10, [ pygame.math.Vector2(100, 100), pygame.math.Vector2(200, 200), pygame.math.Vector2(400, 300)] )
    p1 = None
    p2 = None
    program = True
    while program:
